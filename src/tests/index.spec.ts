...
describe ('test register new user', () => {
  ...
  describe ('test add new product', () => {
    it('should return message: Product created successfully', async () => {
      const response = await request.post('/product').send({
        name: 'test',
        price: 100,
        category: 'test'}).auth(token, {type: 'bearer'});
  ...
  describe ('test update product', () => {
    it('should return message: Product updated successfully', async () => {
      const response = await request.put(`/product/${testProduct.id}`).send({
        name: 'newtest',
        price: 101,
        category: 'newtest'}).auth(token, {type: 'bearer'});
...
describe ('test add new order', () => {
  it('should return message: Order created successfully', async () => {
    const response = await request.post('/order').send({
      ...
      quantity: 1}).auth(token, {type: 'bearer'});
